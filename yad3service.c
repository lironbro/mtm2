		...
		compareMapKeyElements compareKeyElements)
{
	...
	service->userEmails=mapCreate(copyDataElement, copyKeyElement, freeDataElement,
			freeMapKeyElements, compareKeyElements);
	if(service->userEmails == NULL){
		return NULL;
	}
	return service;
}
...
{
	char* type="Realtor";
	if(createUser(type, email, company_name,
			tax_percentage, NULL, NULL, NULL) == NULL){
		return NULL;
	}
	return MTM_SUCCESS;
}
