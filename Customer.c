...
// so I left it as is
yad3Result customerMakeOffer(Customer customer, Realtor realtor, int id, int price,
	...
	if(apartment == NULL)
		return YAD3_APARTMENT_DOES_NOT_EXIST;
	if(!apartmentCorrectProperties(customer, apartment))
		return YAD3_REQUEST_WRONG_PPROPERTIES;
	if(apartmentGetPrice(apartment) > price)	// not sure if I got the instruction right
		return YAD3_REQUEST_ILLOGICAL_PRICE;		// <<<< about this error
