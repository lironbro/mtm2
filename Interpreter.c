...
static bool interpreterRealtorAddApartment(yad3service yad3, char* email,
	...
	int id = atoi((char*)(listGetNext(arguments))),
			...
			width = atoi((char*)(listGetNext(arguments))),
			height = atoi((char*)(listGetNext(arguments)));
	char* matrix = (char*)listGetNext((arguments));
...
static bool interpretCustomerInput(yad3service yad3, char* subcommand,
	...
	if(strcmp(subcommand, "add") == 0){
		...
		return interpreterCustomerMakeOffer(yad3, email, arguments);
	}
	mtmPrintErrorMessage(stderr, MTM_INVALID_COMMAND_LINE_PARAMETERS);
	return true;
}
...
static bool interpretInput(yad3service yad3,
	...
	if(strcmp(command, "realtor") == 0){
		...
		return interpretReportInput(yad3, subcommand, arguments, output);
	}
	mtmPrintErrorMessage(stderr, MTM_INVALID_COMMAND_LINE_PARAMETERS);
	return true;
}
 ...
 *
 * return true if the program needs to be closed
 */
...
static void runProgram(bool close_program, FILE* input, FILE* output){
	...
	while(fgets(line, MAX_LEN, input) != NULL && !close_program
			&& !feof(input)){			// checks that input hasn't ended
...
/*
 * fopenOneFile: does the fopen process on one files, input or output
 *
 * @param input: the input which may be fopened
 * @param output: the output which may be fopened
 * @param argv: the array of arguments which hold the file names
 * @param close_program: key indicating whether the program should be closed
 */
static void fopenOneFile(FILE** input, FILE** output, char** argv,
		bool* close_program){
...
/*
 * fopenTwoFiles: does the fopen process on two files, input and output
 *
 * @param input: the input about to be fopened
 * @param output: the output about to be fopened
 * @param argv: the array of arguments which hold the file names
 * @param close_program: key indicating whether the program should be closed
 */
static void fopenTwoFiles(FILE** input, FILE** output, char** argv,
		bool* close_program){
	if(strcmp(argv[1],"-i") == 0){
		*input = fopen(argv[2], "r");
		fileOpenedProperly(*input, close_program);
...
int main(int argc, char** argv){
	...
	if(argc == 1){	// if input and output aren't given
		...
		fopenOneFile(&input, &output, argv, &close_program);
	}
	else if(argc == 5){	// if both input and output are given
		fopenTwoFiles(&input, &output, argv, &close_program);
	}
	else
		printInvalidCommandLine(&close_program);
