...
bool printErrorCode(yad3Result result){
	...
	if(mtmResult != MTM_INVALID_COMMAND_LINE_PARAMETERS)	// unnecessary test
		mtmPrintErrorMessage(stderr, mtmResult);	// but reminds us that this
	return false;
}
...
static bool runLine(yad3service yad3, char* line, FILE* output){
	...
	while(word != NULL);
	bool stop_running = interpretInput(yad3, command, subcommand,
			arguments, output);
	return stop_running;
}
...
static void runProgram(bool close_program, FILE* input, FILE* output){
	char* line;
	memset(line, '\0', MAX_LEN);	// creates an empty string for fgets
...
/*
 * main1: checks where we should get the input and send the output, attempts to
...
int main(int argc, char** argv){
	...
	else if(argc == 4){	// if both input and output are given
		if(strcmp(argv[0],"i") == 0){
			input = fopen(argv[1], "r");
			fileOpenedProperly(input, &close_program);
		}
