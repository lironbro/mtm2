...
User createUser(char* type, userElement address, char* company_name, char* tax_percentage,
		int min_area, int min_rooms, int max_price)
{
	...
	if(type=="Realtor"){
		Realtor realtor = realtorCreate(company_name, tax_percentage);
		if(realtor==NULL){
			return NULL;
		}
		user->realUser=(userElement*)realtor;
	}
	else{
		Customer customer = customerCreate(min_area, min_rooms, max_price);
		if(customer==NULL){
			return NULL;
		}
		user->realUser=(userElement*)customer;
	}
	return user;
}
