...
void userDestroy(User user)
{
	...
	// free(user->data); // doesn't work, don't know if necessary
	// free(user->type); // doesn't work, don't know if necessary
	free(user);
}
...
User userRealtorCreate(char* company_name, int tax_percentage){
	Realtor temp_realtor = realtorCreate(company_name, tax_percentage);	// don't name this
	if(temp_realtor == NULL)	// "realtor" because it collides with the enum realtor
		return NULL;
	User user = userCreate(realtor, temp_realtor);
	realtorDestroy(temp_realtor);
	return user;
}

User userRealtorCopy(Realtor originalRealtor){
	return userCreate(realtor, realtorCopy(originalRealtor));
}
...
userResult userRealtorRespondToOffer(User userRealtor,
 ...
 bool userIsRealtorRelevant(User user, Customer target){
	 if(user->type != realtor)
		 return false;
	 return realtorIsRealtorRelevant((Realtor)(user->data),
			 customerGetMinArea(target), customerGetMinRooms(target),
			 customerGetMaxPrice(target));
 }
...
User userCustomerCreate(int min_area, int min_rooms, int max_price){
	Customer temp_customer = customerCreate(min_area, min_rooms, max_price);
	if(temp_customer == NULL)
		return NULL;
	User user = userCreate(customer, temp_customer);
	customerDestroy(temp_customer);
	return user;
}

User userCustomerCopy(Customer originalCustomer){
	return userCreate(customer, customerCopy(originalCustomer));
}
