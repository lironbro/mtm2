...
/*
 * intIsInRange: returns whether the num is between min and max, inclusive
 */
...
/*
 * translateMatrix: translates a string into a matrix of squares
 *
 * @param width: the width of the requested matrix
 * @param height: the height of the requested matrix
 * @param matrix: a string representing the square structre of the matrix
 */
 ...
 */
static char* strCopy(char* string)
{
...
char*(*strptr)(char*) = strCopy;

/*
 * freeSquares: frees a matrix of squares
 *
 * @param squares: the matrix in need of freeing
...
Realtor realtorCreate(char* company_name, int tax_percentage){
	if(!intIsInRange(tax_percentage, 1, 100) || company_name==NULL ||
			!strcmp(company_name, "")){
		return NULL;
	}
	...
	realtor->services = mapCreate((copyMapDataElements)serviceCopy,
			(copyMapKeyElements)strptr,
			(freeMapDataElements)serviceDestroy,
			(freeMapKeyElements)strFree, (compareMapKeyElements)strcmp);
	if(realtor->services==NULL){
		return NULL;
	}
	realtor->offers = mapCreate((copyMapDataElements)offerCopy,
			(copyMapKeyElements)strptr,
			(freeMapDataElements)offerDestroy,
			(freeMapKeyElements)strFree, (compareMapKeyElements)strcmp);
	return realtor;
}
...
realtorResult realtorRespondToOffer(Realtor realtor, char* customer_email,
		Choice choice){
...
bool realtorIsRealtorRelevant(Realtor realtor, int customer_min_area,
	...
	MAP_FOREACH(char*, iterator, realtor->services){
		ApartmentServiceResult res = serviceSearch(mapGet(
				realtor->services, iterator),
